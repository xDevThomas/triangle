...
float triangle_size = 5;
int code_words = 32;
int max_flip_sqrt = 80;
float goal_x, goal_y;
...
void improveCode( int baselineX, int baselineY, int baselineCount, float goalX, float goalY ) {
  ...
  for (int iter = 0; iter < iters; iter++) {
    ...
    for (int flips = 0; flips < num_flips; flips++) {
      ...
      switch (r & 3) {
        ...
        case 1:
          ...
          code[rWord] ^= 0xFFFFFFFF;
          break; 

        case 2:
          // Nop. Allows effective entropy per iteration to be lower than maximum,
          // so that more subtle changes can happen too.
          break;
         
        default:
          // Flip bit. This needs to be the most frequent operation for the algorithm to work.
    ...
    if (dist2 < dist1 ||    // A clear improvement in accuracy toward goal
...
void draw() {
  ...
  int t1 = millis();
  improveCode( turtle_x, turtle_y, turtle_triangle_memo.size(), goal_x, goal_y );
  int t2 = millis();
